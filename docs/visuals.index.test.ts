 ...
 *   - every indexed page is on the generated hub (below);
...
const CONTRACT = {
  ...
  /**
   * THE HUB IS THE LEARNER'S ONE BOOKMARK, SO A PAGE THAT IS NOT ON IT IS NOT REACHABLE.
   *
   * `work/visuals/index.html` is generated by scripts/hub.mjs FROM the index above, so the
   * two agree only for as long as somebody re-runs the generator. Upstream the order lived
   * as a clause inside a longer rule with nothing checking it, and on 2026-08-25 a lesson
   * page passed the gate, took its row, was committed and taught — and reaching the board
   * took a commit of its own the next day. Indexed but not on the board leaves a page
   * findable only by remembering its filename.
   *
   * CONTAINMENT, NOT FRESHNESS. Regenerate-and-diff is the obvious check and the wrong one:
   * the hub also prints queue counts and a countdown to the goal date, so it differs from
   * its own regeneration most mornings and would go red on a clean tree while catching
   * nothing. Every page the index links to appears on the board is deterministic, and it is
   * exactly the failure.
   *
   * TWO CAUSES, AND THE MESSAGE NAMES BOTH — this is where mova diverges from upstream,
   * which names only the first. The generator drops a page whose Units cell matches neither
   * a curriculum unit nor the U00 shelf, and reports it on stdout instead. For that page
   * "run hub.mjs" is advice that cannot work, so the failure says to fix the cell.
   */
  hub: "work/visuals/index.html",
  /**
...
describe.skipIf(!active)("work/visuals index", () => {
  ...
  it("every indexed visual is on the generated hub", () => {
    const hubPath = join(root, CONTRACT.hub);
    // No hub yet is a workspace whose generator has never run, not a violation.
    if (!existsSync(hubPath)) return;
    const hub = readFileSync(hubPath, "utf8");
    const absent = linkedFiles().filter((f) => !hub.includes(f));
    expect(
      absent,
      `indexed pages missing from ${CONTRACT.hub}:\n  ${absent.join("\n  ")}\n` +
        `Either the board is stale — run \`node scripts/hub.mjs\` and commit the result — or ` +
        `the row's Units cell names no unit the curriculum defines and no U00, in which case ` +
        `the generator renders the page nowhere and says so on stdout. Fix the cell, then ` +
        `regenerate. Indexing a page without rebuilding the board leaves it reachable only by ` +
        `filename, which is not reachable (media.md → Delivering a visual, rule 3).`,
    ).toEqual([]);
  });
